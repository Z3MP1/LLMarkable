...
def _save_consolidated_output(
    ...
    """
    Save chunks as a single consolidated markdown file without per-chunk metadata.
    ...
    try:
        with output_file.open("w", encoding="utf-8") as f:
            ...
            # Write minimal document metadata if enabled
            if config.include_metadata:
                f.write("---\n")
                f.write(f"**Source:** {input_file.name}\n")
                f.write(f"**Total chunks:** {len(chunks)}\n")
                f.write(
                    f"**Processed:** {datetime.now(UTC).strftime('%Y-%m-%d %H:%M:%S UTC')}\n",
                )
                f.write("---\n\n")

            # Write each chunk content directly without chunk metadata
            for i, chunk in enumerate(chunks):
                # Write content - extract based on chunk type
                if hasattr(chunk, "get"):  # Dictionary-like object
                    content = chunk.get("content", "")
                ...
                # Write clean content without chunk headers
